...
- JS/TS hook factories discovered through `hookCapability` (for example `.ompk/hooks/pre/*.ts`) are loaded as extension modules so their `pi.on(...)` handlers bind to the runtime event bus
