...
- Internal Rust crates remain covered by native architecture docs unless promoted as standalone user-facing commands or APIs. The contributor-facing map lives at [`native-crates.md`](./native-crates.md); today every `crates/*` entry is internal to `@pk-nerdsaver-ai/pi-natives` and the embedded shell, so [`natives-architecture.md`](./natives-architecture.md) and the surrounding native docs own them.
...
- Package: `@pk-nerdsaver-ai/omp-stats`; bin: `omp-stats`; main user path: `omp stats`.
...
- Package: public `@pk-nerdsaver-ai/omptype`; install with `bun add @pk-nerdsaver-ai/omptype`; requires Bun 1.3.14 or newer.
...
- Public surfaces: `@pk-nerdsaver-ai/omptype` for native authoring, `/typebox` and `/zod` for compatibility builders, and `/ark` for the alias-free ArkType compatibility facade.
...
- Package: private `@pk-nerdsaver-ai/typescript-edit-benchmark`; support library with no standalone bin.
...
- Package: private `@pk-nerdsaver-ai/pi-metaharness`; bin: `metaharness`.
...
- Package: private `@pk-nerdsaver-ai/browser-relay`; user command: `omp browser-relay`.
...
- Package: private `@pk-nerdsaver-ai/collab-web`; production client: <https://my.oh-my-pk.pkking.computer/>.
...
- Package: public `@pk-nerdsaver-ai/snapcompact`; install with `bun add @pk-nerdsaver-ai/snapcompact`; requires
  Bun 1.3.14 or newer.
...
- Runtime constraint: rasterization and PNG encoding require `@pk-nerdsaver-ai/pi-natives`.
...
- Package: public `@pk-nerdsaver-ai/pi-mnemopi`; bin: `mnemopi`; requires Bun 1.3.14 or newer. Install globally with `bun add --global @pk-nerdsaver-ai/pi-mnemopi`, then run `mnemopi <command>`. From a source checkout, `bun packages/mnemopi/src/cli.ts <command>` runs the same entrypoint.
