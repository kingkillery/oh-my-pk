...
User remaps live in `~/.ompk/agent/keybindings.yml`. The file is a YAML mapping whose keys are keybinding action IDs and whose values are either one chord string or an array of chord strings. It is not read from `~/.ompk/agent/config.yml`, and there is no nested `keybindings` object.
