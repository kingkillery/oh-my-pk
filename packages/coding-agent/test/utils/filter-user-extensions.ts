...
//    or `../`-relative entry pointing somewhere else under `~/.ompk/`), plus
//    the legacy non-XDG `~/.ompk/plugins` tree on hosts without XDG dirs.
// 2. `getAgentDir()` (`~/.ompk/agent` or `$PI_CODING_AGENT_DIR`)
...
//    Handles installed plugin extensions that live outside `~/.ompk` when
...
// symlinks `~/.ompk/agent/extensions/foo.ts` to a dotfiles repo would see the
