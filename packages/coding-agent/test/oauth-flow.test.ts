...
describe("mcp oauth flow", () => {
	it("uses oh-my-pk client name for dynamic client registration", async () => {
		...
		expect((registrationPayload as { client_name?: string } | null)?.client_name).toBe("oh-my-pk");
			...
			if (url === "https://cropwise.example/oauth/register") {
				return new Response(
					JSON.stringify({
						...
						error_description: "client_name 'oh-my-pk' is not on the approved list.",
					}),
