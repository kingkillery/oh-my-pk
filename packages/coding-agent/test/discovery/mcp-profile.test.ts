 ...
 *
 * A named profile relocates the agent directory to ~/.ompk/profiles/<name>/agent.
 * The native config provider used to read user-scope mcp.json from the literal
 * home (~/.ompk/agent/mcp.json) via `ctx.home`, so a profile never saw its own
 ...
 *
 * `os.homedir()` is mocked so the *old* code path (ctx.home + ".ompk/agent")
...
describe("native user-level MCP discovery follows the active profile", () => {
	...
	test("active profile loads its own user server, not the default profile's", async () => {
		// Active profile's agent dir (stand-in for ~/.ompk/profiles/<name>/agent).
		...
		await writeMcpJson(path.join(tempHome, ".ompk", "agent"), {
	...
	test("default profile loads the user server from ~/.ompk/agent", async () => {
		const defaultAgentDir = path.join(tempHome, ".ompk", "agent");
