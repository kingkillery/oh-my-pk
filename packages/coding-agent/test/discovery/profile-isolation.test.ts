 ...
 * profile. A profile relocates the agent directory to ~/.ompk/profiles/<name>/agent;
 * the native provider used to read user config (commands, skills, rules, etc.)
 * from the literal home (~/.ompk/agent) via `ctx.home`, leaking the default
 ...
 * (ctx.home + ".ompk/agent") points at the tempdir decoys below; without the fix
