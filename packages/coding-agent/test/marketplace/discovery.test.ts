 ...
 * depends on:
 *   1. OMP registry lives at path.join(home, ".ompk", "plugins", "installed_plugins.json")
 *      (matches getConfigDirName() == ".ompk")
...
// but we know the value is ".ompk" and hardcoding it here keeps tests free of
// native-addon transitive imports.
const OMP_CONFIG_DIR = ".ompk";
...
let tmpHome: string;
/** ~/.ompk/plugins/installed_plugins.json inside tmpHome */
...
describe("OMP registry path contract", () => {
	it("OMP registry lives at home/.ompk/plugins/installed_plugins.json", () => {
		// This is the path that listClaudePluginRoots reads.
		// Any change to this path must be reflected in helpers.ts.
		const expected = path.join(tmpHome, ".ompk", "plugins", "installed_plugins.json");
