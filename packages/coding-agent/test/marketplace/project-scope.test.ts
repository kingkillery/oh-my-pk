...
describe("resolveActiveProjectRegistryPath", () => {
	...
	it("walk-up finds nearest .ompk/ directory", async () => {
		// Layout: tmpDir/.ompk/   +   tmpDir/sub/nested/  (cwd)
		// Resolver must climb from cwd → sub → tmpDir and find .ompk/ there.
		fs.mkdirSync(path.join(tmpDir, ".ompk"), { recursive: true });
		const cwd = path.join(tmpDir, "sub", "nested");
		fs.mkdirSync(cwd, { recursive: true });

		const result = await resolveActiveProjectRegistryPath(cwd);

		expect(result).toBe(path.join(tmpDir, ".ompk", "plugins", "installed_plugins.json"));
	});

	it("walk-up stops at the nearest .ompk/ — does not skip to a more distant one", async () => {
		// Layout: tmpDir/.ompk/   +   tmpDir/sub/.ompk/   +   tmpDir/sub/nested/  (cwd)
		// Resolver must stop at tmpDir/sub/.ompk/, not climb further to tmpDir/.ompk/.
		fs.mkdirSync(path.join(tmpDir, ".ompk"), { recursive: true });
		fs.mkdirSync(path.join(tmpDir, "sub", ".ompk"), { recursive: true });
		const cwd = path.join(tmpDir, "sub", "nested");
		fs.mkdirSync(cwd, { recursive: true });

		const result = await resolveActiveProjectRegistryPath(cwd);

		expect(result).toBe(path.join(tmpDir, "sub", ".ompk", "plugins", "installed_plugins.json"));
	});

	it("falls back to .git root when no .ompk/ exists", async () => {
		// Layout: tmpDir/.git/   +   tmpDir/sub/  (cwd)
		// No .ompk/ anywhere → second pass finds .git/ at tmpDir.
	...
	it("returns null when neither .ompk/ nor .git/ found anywhere in the tree", async () => {
		// Start at the filesystem root — guaranteed to have no .ompk/ or .git/ ancestors.
...
describe("listClaudePluginRoots — project shadows user", () => {
	...
	beforeEach(() => {
		...
		// Create .ompk/ in project so resolveActiveProjectRegistryPath finds it.
		fs.mkdirSync(path.join(tmpProject, ".ompk", "plugins"), { recursive: true });

		userRegPath = path.join(tmpHome, ".ompk", "plugins", "installed_plugins.json");
		fs.mkdirSync(path.dirname(userRegPath), { recursive: true });

		projectRegPath = path.join(tmpProject, ".ompk", "plugins", "installed_plugins.json");
	});
