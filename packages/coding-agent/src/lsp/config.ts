...
import { $which, isRecord, logger, pathIsWithin, type WhichOptions } from "@pk-nerdsaver-ai/pi-utils";
...
function getConfigSources(cwd: string): ConfigSource[] {
	...
	// Project config directories (.ompk/, .pi/, .claude/)
	...
	// User config directories (~/.ompk/agent/, ~/.pi/agent/, ~/.claude/)
 ...
 * 2. Project config dirs: .ompk/lsp.*, .pi/lsp.*, .claude/lsp.* (+ hidden variants)
 * 3. User config dirs: ~/.ompk/agent/lsp.*, ~/.pi/agent/lsp.*, ~/.claude/lsp.* (+ hidden variants)
