...
#
# Build:
#     docker build -t oh-my-pk/pi:dev .                          # default = pi-runtime
#     docker build --target pi-base -t oh-my-pk/pi-base:dev .    # base for derived images
#
# Run:
#     docker run --rm oh-my-pk/pi:dev --help
#     docker run --rm -it -v "$PWD":/work oh-my-pk/pi:dev cli    # interactive omp
#
# Consume as a base in another Dockerfile (see Dockerfile.robomp):
#     ARG PI_BASE=oh-my-pk/pi:dev
...
#
# A self-contained, runnable omp image. `docker run oh-my-pk/pi:dev --help`
