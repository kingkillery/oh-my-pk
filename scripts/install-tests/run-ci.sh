...
omptype_tgz="$(find_tarball "$TARBALL_DIR"/pk-nerdsaver-ai-omptype-*.tgz)"
...
(
   ...
   node -e "
		...
		pkg.overrides = {
			...
			'@pk-nerdsaver-ai/omptype': '$omptype_tgz',
   ...
   [ "$omptype_probe" = "omp:false:tb" ] || {
      echo "Unexpected @pk-nerdsaver-ai/omptype probe result: $omptype_probe"
      exit 1
   }
