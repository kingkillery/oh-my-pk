...
const ARCHIVE_LINK =
	"Older entries are archived in [packages/foo/CHANGELOG.md@abc123def456](https://github.com/kingkillery/oh-my-pk/blob/abc123def456abc123def456abc123def456abc1/packages/foo/CHANGELOG.md).";
...
describe("runChangelogFixer size limit", () => {
	it("collapses oversized changelogs behind a link to the last commit containing them, idempotently", async () => {
		...
		try {
			...
			const repo = process.env.OMP_REPO ?? process.env.GITHUB_REPOSITORY ?? "kingkillery/oh-my-pk";
