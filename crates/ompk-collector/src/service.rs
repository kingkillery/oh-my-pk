...
pub fn install(spec: &ServiceSpec) -> Result<String> {
	match std::env::consts::OS {
		...
		"linux" => {
			...
			run(std::process::Command::new("systemctl").args([
				"--user",
				"enable",
				"--now",
				format!("{SERVICE_LABEL}.service").as_str(),
			]))?;
			Ok(format!(
				"Installed systemd user unit {path} (enabled now and at boot). If the service stops \
				 when you log out, run `loginctl enable-linger $USER` once."
			))
		},
		"windows" => {
	...
	match std::env::consts::OS {
		...
		"windows" => {
			...
			match out {
				...
				Ok(o) => bail!("reg delete failed: {}", String::from_utf8_lossy(&o.stderr).trim()),
...
pub fn installed_marker() -> bool {
	match std::env::consts::OS {
		"macos" => home().is_ok_and(|h| {
			std::path::Path::new(&h)
				.join(format!("Library/LaunchAgents/{SERVICE_LABEL}.plist"))
				.exists()
		}),
		"linux" => home().is_ok_and(|h| {
			std::path::Path::new(&h)
				.join(format!(".config/systemd/user/{SERVICE_LABEL}.service"))
				.exists()
		}),
...
mod tests {
	...
	fn plist_carries_serve_arguments_and_logs() {
		...
		assert_eq!(
			p.matches("<string>/usr/local/bin/ompk-collector</string>")
				.count(),
			1
		);
	}
	...
	fn systemd_unit_quotes_exec_start() {
		...
		assert!(u.contains(
			"ExecStart=\"/usr/local/bin/ompk-collector\" \"--dir\" \"/Users/k/.ompk\" \"serve\""
		));
