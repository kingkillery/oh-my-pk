...
fn insert_from(conn: &Connection, item: &Value, env: &Envelope<'_>) -> Result<(i64, bool)> {
    store::insert_report(conn, &str_of(item, "tool"), &str_of(item, "report"), &str_of(item, "model"), &str_of(item, "version"), env)
}
...
fn auto_file(state: Arc<State>, repo: String, groups: BTreeSet<String>) {
    for g in groups {
        ...
        if let Err(e) = triage::ensure_repo_issue(&conn, &state.cfg.triage, &repo, &g) {
            eprintln!("{}", json!({ "auto_file": "error", "repo": repo, "group": g, "error": clean_text(&e.to_string(), 300) }));
        }
    }
}
...
fn handle(state: &Arc<State>, mut req: Request) {
    ...
    let peer = req.remote_addr().map(|a| a.ip().to_string()).unwrap_or_default();
    ...
    match (method, path.as_str()) {
        (Method::Get, "/status") => {
            let resp = match status_payload(state) {
                ...
                Err(e) => json_response(500, &json!({ "ok": false, "error": clean_text(&e.to_string(), 300) })),
            };
        ...
        (Method::Get, "/triage") => {
            ...
            let resp = match result {
                ...
                Err(e) => json_response(502, &json!({ "ok": false, "error": clean_text(&e.to_string(), 300) })),
            };
            let _ = req.respond(resp);
        }
        (Method::Post, "/v1/report") => {
            let payload = match read_json(&mut req) {
                Ok(v) => v,
                Err(e) => {
                    let _ = req.respond(json_response(400, &json!({ "ok": false, "error": clean_text(&e.to_string(), 200) })));
                    return;
                }
            };
            ...
            let resp = match result {
                Ok((id, inserted)) => json_response(202, &json!({ "ok": true, "id": id, "inserted": inserted })),
        ...
        (Method::Post, "/v1/reports" | "/v1/grievances" | "/") => {
            ...
            let items = payload.get("entries").or_else(|| payload.get("reports")).and_then(Value::as_array);
            let Some(items) = items else {
                let _ = req.respond(json_response(400, &json!({ "ok": false, "error": "entries array required" })));
                return;
            };
            ...
            let env = Envelope { source: &peer, platform: &platform, arch: &arch, install_id: &install_id };
                ...
                for item in items.iter().take(MAX_ENTRIES) {
                    ...
                    match insert_from(&conn, item, &env) {
                        Ok((id, inserted)) => {
                            ...
                            if inserted
                                && let Ok(Some(g)) = store::group_id_of(&conn, id) {
                                    new_groups.insert(g);
                                }
                        }
            ...
            .spawn(move || {
                loop {
                    ...
                    match result {
                        ...
                        Err(e) => eprintln!("{}", json!({ "triage": "error", "error": clean_text(&e.to_string(), 300) })),
                    }
                }
            })
            .ok();
