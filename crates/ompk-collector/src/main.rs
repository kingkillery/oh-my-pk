...
//! - **local extension**: spawned by OMPK when `dev.autoqa.collector.mode =
//!   local`
//! - **standalone helper**: installed on a server you control; prints the URL +
//!   token to paste into OMPK's remote-collector settings
...
#[command(
	name = "ompk-collector",
	version,
	about = "OMPK tool-issue collector: ingest, dedupe, rank, file GitHub issues"
)]
struct Cli {
...
enum Cmd {
	...
	Serve {
		/// Bind address (0.0.0.0 to accept LAN/tailnet clients, 127.0.0.1 for
		/// local-only)
	...
	Token,
	/// Print the pairing block (URL + token + paste-ready OMPK command) without
	/// serving
	PairInfo {
		...
		Cmd::Install { bind, port, github_repo, issue_limit, print } => {
			let spec = service::ServiceSpec {
				binary: std::env::current_exe()
					.map_or_else(|_| "ompk-collector".into(), |p| p.display().to_string()),
