...
pub fn existing_issue_map(repo: &str) -> Result<HashMap<String, String>> {
    ...
    for page in 1..=5 {
        let items = request("GET", &format!("/repos/{repo}/issues?state=all&per_page=100&page={page}"), None)?;
...
pub fn create_issue(repo: &str, title: &str, body: &str) -> Result<CreatedIssue> {
    let v = request("POST", &format!("/repos/{repo}/issues"), Some(json!({ "title": title, "body": body })))?;
    let url = v.get("html_url").and_then(Value::as_str).context("missing html_url")?.to_string();
    let number = v.get("number").and_then(Value::as_i64).context("missing number")?;
