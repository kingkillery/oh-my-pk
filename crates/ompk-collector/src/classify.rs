...
const GROUP_DEFS: &[(&str, &str, u32, &[&str])] = &[
    ...
    ("edit-anchor-safety", "edit accepts stale or unseen anchors and can target unintended code", 98, &[
        r"stale.*anchor",
        r"unseen.*anchor",
    ...
    ("edit-syntax-persistence", "edit persists patch-control syntax or rendered snapshot text", 96, &[
    ...
    ("edit-block-resolution", "fresh edit anchors are rejected or Rust attributes do not resolve with blocks", 80, &[
        r"fresh.*anchor.*reject",
        r"anchor.*reject.*fresh",
        r"#\[test\].*(?:SWAP\.BLK|block|attribute)",
        r"attribute.*(?:block|function).*resolv",
    ]),
    ("task-terminal-state", "successful task completion is reopened or reported as cancelled", 86, &[
    ...
    ("capability-advertising", "advertised agents, tools, or model capabilities differ from runtime availability", 76, &[
    ...
    ("irc-delivery", "IRC wake, completion addressing, and follow-up delivery disagree with peer state", 68, &[
    ...
    ("ix-browser-state", "IX Bridge lane leases and navigation target stale or wrong browser state", 72, &[
        r"ix_bridge.*(?:lane|lease|tab|window|navigation|stale|wrong)",
        r"browser.*(?:lease|lane|stale|wrong).*(?:tab|window|url)",
    ]),
    ("browser-fill", "browser fill and displayed extraction results do not match documented behavior", 66, &[
        r"browser.*(?:fill|contenteditable|input event|extraction|snapshot)",
        r"fill.*(?:empty|placeholder|input event)",
    ]),
    ("eval-js-bindings", "JavaScript top-level bindings disappear between successful eval cells", 88, &[
        r"javascript.*(?:binding|top-level|var|let|function).*(?:disappear|ReferenceError|missing)",
        r"eval.*js.*(?:binding|state|ReferenceError)",
    ]),
    ("eval-shared-reset", "shared Python resets invalidate another agent's persistent state", 78, &[
        r"python.*(?:reset|state).*(?:agent|peer|shared)",
        r"shared.*kernel.*reset",
        r"peer reset.*kernel",
    ]),
    ("eval-hang", "lightweight Python cells and subprocess calls hang until watchdog termination", 90, &[
    ...
    ("windows-path-identity", "Windows path identity and cached content disagree across tools", 74, &[
    ...
    ("bash-contract", "bash cwd, timeout limits, and failure artifacts differ from the contract", 62, &[
    ...
    ("adapter-representation", "URL and document adapters return stale or wrong resource representations", 60, &[
        r"url.*(?:stale|metadata|raw source|cache)",
        r"xlsb|png.*text|sqlite.*handle|pagination.*long line",
        r"hugging ?face.*(?:metadata|source)",
    ]),
    ("resource-resolution", "advertised vault and skill resources are not reliably resolvable", 58, &[
        r"skill.*(?:file|resource).*(?:missing|unresolvable|read)",
        r"vault.*(?:search|active|resolve|unsupported|exit 255)",
    ]),
    ("startup-preflight", "tool startup needs capability-aware model and sandbox validation", 56, &[
...
static NOISE: LazyLock<Vec<Regex>> = LazyLock::new(|| NOISE_PATTERNS.iter().map(|p| ci(p)).collect());
...
static VERSION_RE: LazyLock<Regex> =
    LazyLock::new(|| Regex::new(r"\b(?:v?\d+\.\d+\.\d+|#[0-9a-f]{4,}|id\s*\d+)\b").expect("version regex"));
static NON_ALNUM_RE: LazyLock<Regex> = LazyLock::new(|| Regex::new(r"[^a-z0-9_]+").expect("alnum regex"));
...
mod tests {
    ...
    fn groups_related_wording_but_keeps_distinct_fingerprints() {
        let a = classify("read returned the wrong file contents for concurrent requests; results were swapped");
        let b = classify("concurrent artifact reads produced wrong output attributed to another request");
