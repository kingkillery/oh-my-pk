...
use crate::classify::{classify, clean_text, MAX_REPORT_CHARS};
...
pub fn insert_report(
    ...
    let install_hash = if env.install_id.is_empty() {
        ...
        h.finalize().iter().take(8).fold(String::new(), |mut acc, b| {
    ...
    let res = conn.execute(
        "INSERT INTO reports(received_at, source, model, version, tool, report,
            platform, arch, install_hash, group_id, group_title, impact, noise, normalized, fingerprint)
    ...
    match res {
        ...
        Err(rusqlite::Error::SqliteFailure(e, _)) if e.code == rusqlite::ErrorCode::ConstraintViolation => {
            let id: i64 = conn
                .query_row("SELECT id FROM reports WHERE fingerprint = ?1", params![c.fingerprint], |r| r.get(0))
...
const GROUP_SELECT: &str = "SELECT group_id, group_title, MAX(impact), COUNT(*), COUNT(DISTINCT tool),
...
pub fn group_rows(conn: &Connection) -> Result<Vec<GroupRow>> {
    ...
    let rows = st.query_map([], map_group)?.collect::<std::result::Result<Vec<_>, _>>()?;
    Ok(rows)
}
...
pub fn group_row(conn: &Connection, group_id: &str) -> Result<Option<GroupRow>> {
    ...
    Ok(conn.query_row(&sql, params![group_id], map_group).optional()?)
}
...
pub fn counts(conn: &Connection) -> Result<(i64, i64)> {
    ...
    let noise: i64 = conn.query_row("SELECT COUNT(*) FROM reports WHERE noise = 1", [], |r| r.get(0))?;
    Ok((total, noise))
}
...
mod tests {
    ...
    fn mem() -> Connection {
        ...
        // Reuse schema path via a temp file would need fs; replicate via open on ":memory:" is not
        // supported by `open`, so run the DDL directly.
        c.execute_batch(
            "CREATE TABLE reports (id INTEGER PRIMARY KEY AUTOINCREMENT, received_at TEXT NOT NULL,
             ...
             install_hash TEXT NOT NULL DEFAULT '', group_id TEXT NOT NULL, group_title TEXT NOT NULL,
    ...
    fn exact_duplicate_collapses() {
        ...
        let (id1, ins1) =
            insert_report(&c, "read", "read returned wrong file contents for concurrent requests", "m", "1", &env)
                .unwrap();
        let (id2, ins2) =
            insert_report(&c, "read", "read returned wrong file contents for concurrent requests", "m", "1", &env)
                .unwrap();
    ...
    fn install_id_is_hashed_not_stored() {
        let c = mem();
        let env = Envelope { source: "t", install_id: "very-secret-install-id", ..Default::default() };
        insert_report(&c, "read", "some report about wrong file", "m", "1", &env).unwrap();
        let stored: String = c.query_row("SELECT install_hash FROM reports", [], |r| r.get(0)).unwrap();
    ...
    fn groups_rank_by_impact() {
        let c = mem();
        let env = Envelope::default();
        insert_report(&c, "edit", "edit accepts stale anchor and modifies unintended shifted code", "m", "1", &env)
            .unwrap();
        insert_report(&c, "read", "vault search fails unsupported on Windows", "m", "1", &env).unwrap();
