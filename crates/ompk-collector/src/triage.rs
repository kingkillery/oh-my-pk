...
use crate::classify::clean_text;
use crate::github::{self, TITLE_PREFIX};
use crate::store::{self, GroupRow};
...
impl Published {
    ...
    pub fn set(&mut self, repo: &str, group_id: &str, url: &str) {
        self.map.insert(format!("{repo}#{group_id}"), Value::String(url.to_string()));
    }
...
This issue was selected by the OMPK QA collector as one of the top {issue_limit} impact-ranked report groups. It is a **sanitized aggregate of agent-reported symptoms**, not proof that the defect still exists on current `main`.
...
fn publish(conn: &Connection, cfg: &TriageConfig, repo: &str, row: &GroupRow) -> Result<String> {
    ...
    let created = github::create_issue(repo, &issue_title(row), &issue_body(conn, row, cfg.issue_limit)?)?;
...
pub fn ensure_repo_issue(conn: &Connection, cfg: &TriageConfig, repo: &str, group_id: &str) -> Result<Option<String>> {
    ...
    let existing = if cfg.dry_run { HashMap::new() } else { github::existing_issue_map(repo)? };
...
pub fn triage(conn: &Connection, cfg: &TriageConfig) -> Result<Vec<Selected>> {
    ...
    let mut published = Published::load(&cfg.published_path);
    let existing = if cfg.dry_run { HashMap::new() } else { github::existing_issue_map(&cfg.github_repo)? };
    let mut out = Vec::new();
...
mod tests {
    ...
    fn published_map_reads_legacy_bare_keys_as_default_repo() {
        ...
        std::fs::write(&path, r#"{"bash-contract":"https://x/issues/65","o/r#g":"https://x/issues/1"}"#).unwrap();
        let p = Published::load(&path);
        assert_eq!(p.get("default/repo", "default/repo", "bash-contract").as_deref(), Some("https://x/issues/65"));
    ...
    fn dry_run_triage_ranks_and_limits() {
        ...
        insert_report(&conn, "edit", "edit accepts stale anchor and modifies unintended shifted code", "m", "1", &env)
            .unwrap();
        insert_report(&conn, "read", "vault search fails unsupported on Windows", "m", "1", &env).unwrap();
    ...
    fn ensure_repo_issue_dedups_per_repo() {
        let dir = tmp("ensure");
        let conn = store::open(&dir.join("c.db")).unwrap();
        insert_report(&conn, "bash", "bash cwd resolved against wrong checkout", "m", "1", &Envelope::default())
            .unwrap();
        let cfg = TriageConfig {
            github_repo: "x/y".into(),
            issue_limit: 10,
    ...
    fn issue_body_is_sanitized_and_titled() {
        ...
        .unwrap();
        let row = store::group_row(&conn, "result-attribution").unwrap().unwrap();
